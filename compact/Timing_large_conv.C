...
void Timing_large_conv(int angle, float cut)
{
 std:: ofstream C_SiPM;
C_SiPM.open("Channel_0116_100.csv");
  ...
  TH1F *hcEcaln_other[nchan]; //Scintillation Generated and also for all the SiPMs
  ...
  {   
   ...
   if(i>=2 && i<=9)
   {
    ...
    hcEcaln_other[i] = new TH1F(Form("hcEcaln_other_%d",i),"total number of S+C chan 1",1000,0.,100);
    h_conv[i] = new TH1F(Form("h_conv_%d",i),Form("Convolved signal for Ch %i",i-2),1000,0.,100);
   ...
   hcEcaln_other[i] = new TH1F(Form("hcEcaln_other_%d",i),"total number of S+C chan 1",7000000,0.,7000000);
   h_conv[i] = new TH1F(Form("hcEcaln_other_%d",i),"total number of S+C chan 1",7000000,0.,7000000);}
 ...
 std::vector<float> Ch_other_avg[8];
 ...
 TTree *Timing_channel = (TTree *)f->Get("Timing_channel");
  ...
  std::vector<float> *Ch_other[8];
  for(int i=0;i<8;i++)
  {
   Ch[i]=new vector<float>;
   Ch_other[i]=new vector<float>;
  }
  ...
  for(int i=0;i<8;i++)
  {
   ...
   Timing_channel->SetBranchAddress(Form("Ch_other_%i",i),&Ch_other[i]);
  }
    ...
    for(int k=0;k<8;k++)
    {
     ...
     for(int j=0;j<Ch[k]->size();j++)
     {
       if(i==0)
       {
        ...
        Ch_other_avg[k].push_back(0.0);
       }
     cur_evt.Ch[k].push_back(Ch[k]->at(j));
     cur_evt.Ch_other[k].push_back(Ch_other[k]->at(j));
     }
  ...
  for(int i=0;i<Timing_channel->GetEntries();i++)
  {
   if(evt_80.at(i).gen_tot>=xcer0)
   {
    ...
    for(int j=0;j<Ch_avg[k].size();j++)
    {
     ...
     Ch_other_avg[k].at(j)+=evt_80.at(i).Ch_other[k].at(j);
    }
    }
   }
  }
   ...
   std::cout << "Number of events passing the cut " << icut << std::endl;
    ...
    for(int j=0;j<Ch_avg[k].size();j++)
    {
     ...
     Ch_other_avg[k].at(j) = Ch_other_avg[k].at(j)/icut;
    }
    }
    ...
    for(int j=0;j<Ch_avg[k].size();j++)
    {
     ...
     hcEcaln_other[k+2]->SetBinContent(j+1,Ch_other_avg[k].at(j));
    }
    }

  int i_plot = 9;
  int i_plot_2 = 2;
  hcEcalntot[i_plot]->Draw("");
  hcEcalntot[i_plot]->SetStats(0);
  hcEcalntot[i_plot]->SetLineColor(kGreen);
  /*
  hcEcaln_other[i_plot]->Draw("same");
  hcEcaln_other[i_plot]->SetStats(0);
  hcEcaln_other[i_plot]->SetLineColor(kRed);
  */
  /*
  hcEcalntot[i_plot]->GetXaxis()->SetTitle("Time (in ns)");
  hcEcalntot[i_plot]->GetYaxis()->SetTitle("Number of counts");
  */
  stringstream j_ch,j_angle, j_cut;
  string ch_str, angle_str, cut_str;
  j_ch << i_plot-2;
  j_ch >> ch_str;
  j_angle << angle;
  j_angle >> angle_str;
  j_cut << cut*100;
  j_cut >> cut_str;
  //else if(cut==1.0){j_cut >> cut_str;}

  string title_string; 
  if(cut<1.0){title_string = "Ch " + ch_str +" for " + angle_str + " deg with " + cut_str + " % cut";}
  else if (cut==1.0){title_string = "Ch " + ch_str +" for " + angle_str + " deg without cuts";}
  hcEcalntot[i_plot]->SetTitle(title_string.c_str());
  ...
  {
   h_spr->SetBinContent(j+1,single_photon_response.at(j));
   std::cout <<"Bin number " << j << " Value of spr " << single_photon_response.at(j) <<std::endl;
  }

  h_spr->Draw("same");
  h_spr->SetLineColor(kRed);
  float conv_j;
  ...
  for(int j=0;j<single_photon_response.size();j++)
  {
   h_conv[k+2]->SetBinContent(j+1,convolved_signal[k].at(j));
  }
 }

 double max_val_1 = h_conv[i_plot]->GetBinContent(h_conv[i_plot]->GetMinimumBin());
 double max_val_2 = h_conv[i_plot_2]->GetBinContent(h_conv[i_plot_2]->GetMinimumBin());
std::cout <<"Mod of Maximum values " << max_val_1 << " " << max_val_2 << std::endl;
  for(int j=0;j<single_photon_response.size();j++){h_conv[i_plot_2]->SetBinContent(j+1,convolved_signal[i_plot_2-2].at(j)*max_val_1/max_val_2);}

 TCanvas *c1=new TCanvas("c_1","Graph Draw Options", 200,10,600,400); //represents coordinates of start and end points of canvas
 h_conv[i_plot]->Draw("");
 h_conv[i_plot]->SetLineColor(kBlue);
 h_conv[i_plot]->SetStats(0);


 h_conv[i_plot_2]->Draw("same");
 h_conv[i_plot_2]->SetLineColor(kRed);
 h_conv[i_plot_2]->SetStats(0);

 h_conv[i_plot]->GetXaxis()->SetTitle("Time (in ns)");
 h_conv[i_plot]->GetYaxis()->SetTitle("Number of p.e. counts * mV");
 //c1->SetTitle(Form("Convolved signal comparison for %i deg",angle));
