...
const int ichan[nchan] = {257,256,262,260,263,261,264,266,265,267,258,259,268}; //new geometry without layer box, with filter
std::string namechan[nchan] = {"crystal","Air","Ch 0","Ch 1","Ch 2","Ch 3","Ch 4","Ch 5","Ch 6","Ch 7","Left Silicone Cookie","Right Silicone Cookie","R60 Filter"};
...
void Timing_large_dist(int angle/*int num_evtsmax, const char* inputfilename*/) {
  ...
  for (int i=0;i<nchan;i++) //Although the range of the x axis on the histograms will be much smaller for the SiPM channels
  {
   ...
   gtottime[i] = new TH1F(Form("gtottime_%d",i),"gscinttime",1000,0,100);
   ...
   gtottimecut[i] = new TH1F(Form("gtottime_%d",i),"gscinttime",1000,0,100);
...
std::cout<<std::endl;
  ...
  else
  {
    ...
    for(int j=0;j<ncerwave_dummy[0].size();j++) //Right now all sizes are the same = binsize, taken the channel 0
    {
     ...
     filter[10].push_back(1.0);
     filter[11].push_back(1.0);
     filter[12].push_back(1.0);
   ...
   //if(ievt==894){std::cout << "Problematic event " << ievt << " "<< ncerwavechan[0].size() << std::endl;}    
     ...
     for(int k_t=0; k_t<ncerwave_timechan[k].size(); k_t++)
     {
      ...
      for(int k_t=0; k_t<ncerwave_timechan[0].size(); k_t++)
      {
      Gen_crys->push_back(ncertimecutchan[0].at(k_t)+nscinttimecutchan[0].at(k_t));
      for(int i=0;i<8;i++)
      {
       Ch[i]->push_back(ncertimecutchan[i+2].at(k_t)+nscinttimecutchan[i+2].at(k_t));
      }
      }

      Timing_channel->Fill();

      std::cout << "Size of gen time signal for each event " << Gen_crys->size() << std::endl;
      Gen_crys->clear();
      for(int i=0;i<8;i++)
      {
       Ch[i]->clear();
...
//  }  // end if no events - Not needed since num_evt is already specified
    ...
    string Entry_index_time_gen ="Plots/PbWO4/1207/Angle_"+ entry_filter_string + "_deg/" + " New Time distribution of generated photons for " + entry_filter_string + " deg.pdf";
    string time_gen = " New Time distribution of generated photons for " + entry_filter_string + " deg";
    string Entry_index_wave_time_gen ="Plots/PbWO4/1207/Angle_"+ entry_filter_string + "_deg/" + " New wavelength and Time distribution of generated photons for " + entry_filter_string + " deg.pdf";
    string wave_time_gen = " New wavelength and Time distribution of generated photons for " + entry_filter_string + " deg";
    ...
    string Entry_index_filter_time_cut ="Plots/PbWO4/1207/Angle_"+ entry_filter_string + "_deg/" + side_string + " SiPM " + title_string + " New Time distribution with SiPM and filter for " + entry_filter_string + " deg.pdf";
    string filter_time = side_string + " SiPM " + title_string + " New Time distribution with SiPM and filter for " + entry_filter_string + " deg";

    string Entry_index_time ="Plots/PbWO4/1207/Angle_"+ entry_filter_string + "_deg/" + side_string + " SiPM " + title_string + " New Time distribution for " + entry_filter_string + " deg.pdf";
    string time = side_string + " SiPM " + title_string + " New Time distribution for " + entry_filter_string + " deg";

    string Entry_index_wave_time_cut ="Plots/PbWO4/1207/Angle_"+ entry_filter_string + "_deg/" + side_string + " SiPM " + title_string + " New wavelength and Time distribution with SiPM and filter for " + entry_filter_string + " deg.pdf";
    string wave_time_cut = side_string + " SiPM " + title_string + " New wavelength and Time distribution with SiPM and filter for " + entry_filter_string + " deg";

    string Entry_index_wave_time ="Plots/PbWO4/1207/Angle_"+ entry_filter_string + "_deg/" + side_string + " SiPM " + title_string + " New wavelength and Time distribution for " + entry_filter_string + " deg.pdf";
    string wave_time = side_string + " SiPM " + title_string + " New wavelength and Time distribution for " + entry_filter_string + " deg";
